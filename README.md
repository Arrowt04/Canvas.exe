# Canvas.exe - Android GDI & Bytebeat Simulator

A harmless, educational 2-dimensional graphics rendering project built for Android using local Canvas pipelines and real-time AudioTrack procedural synthesis.

## ⚠️ Disclaimer
This application is COMPLETELY HARMLESS joke software developed for educational, testing, and video performance purposes. 
* It does NOT steal personal files or monitor user data.
* It does NOT cause permanent hardware or software damage.
* It creates a temporary graphical lag and flashes screen buffers to simulate old desktop artifact pranks.
* **Termination:** Simply tap the screen 100 times to terminate all core loops and safely restore your original wallpaper.

## 🛠️ Features
* **Z-Index Layering:** Smooth rendering with particles flying over complex grid matrices.
* **Kinetic Vector Triangles:** Spin-stabilized nodes leaving colorful trail attenuation.
* **Real-time Bytebeat:** Generates retro 8-bit saw sounds directly from mathematical data (`t & t >> 8`).
